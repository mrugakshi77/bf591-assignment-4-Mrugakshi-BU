...
knitr::kable(head(filtered_verse_counts))
```
...
metadata <- meta_info_from_labels(colnames(filtered_verse_counts[2:length(colnames(filtered_verse_counts))]))
metadata
```
...
library_size <- get_library_size(filtered_verse_counts)
#Make a barplot showing the number of millions of reads for each sample
pivot_longer(library_size/10^6, everything(), names_to="Sample",values_to="# reads") %>%
  mutate(Sample=factor(Sample,levels=colnames(library_size))) %>%
...
cpm <- normalize_by_cpm(filtered_verse_counts)
...
deseq2 <- deseq_normalize(filtered_verse_counts, metadata)
knitr::kable(head(deseq2))
```
...
plot_sample_distributions(filtered_verse_counts[c(-1)], TRUE, "Count Distributions using log10(counts)")
```
...
plot_variance_vs_mean(filtered_verse_counts[c(-1)], TRUE, "Variance vs. mean count using log10(counts)")
```
...
plot_pca(filtered_verse_counts[c(-1)], metadata, "PCA plot using log10(counts)")
```
...
plot_sample_distributions(cpm[c(-1)], TRUE, "CPM normalized Count Distributions using log10(counts)")
plot_variance_vs_mean(cpm[c(-1)], TRUE, "CPM - Variance vs. mean count using log10(counts)")
plot_pca(cpm[c(-1)], metadata, "CPM PCA plot using log10(counts)")
```
...
plot_sample_distributions(deseq2[c(-1)], TRUE, "Deseq2 normalized Count Distributions using log10(counts)")
plot_variance_vs_mean(deseq2[c(-1)], TRUE, "Deseq2 - Variance vs. mean count using log10(counts)")
plot_pca(deseq2[c(-1)], metadata, "Deseq2 PCA plot using log10(counts)")
```
